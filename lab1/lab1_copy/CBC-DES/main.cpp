...
int main()
{
    ...
    {
        fstream fin, fout;
        fin.open("F:\\CBC-DES\\input.txt", ios_base::in); // 这里和下面文件均是绝对路径，注意修改
        fout.open("F:\\CBC-DES\\output.txt", ios_base::out);
        while (!fin.eof())
        {
    ...
    {
        fstream fin, fout;
        fin.open("F:\\CBC-DES\\output.txt", ios_base::in);
        fout.open("F:\\CBC-DES\\plainText.txt", ios_base::out);
        while (!fin.eof())
        {
            getline(fin, text);
            int count = static_cast<int>(text.length()) / 8;
